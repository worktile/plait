...
describe('vector', () => {
    ...
    describe('get point on vector by vector and vector component', () => {
        it('get point by horizontal component', () => {
            ...
            const unitVector = getUnitVectorByPointAndPoint(start, end);
            const result1 = getPointByVectorDirectionComponent(start, unitVector, horizontalComponent, true);
            expect(result1[0]).toEqual(150);
            expect(result1[1]).toEqual(75);
            const result2 = getPointByVectorDirectionComponent(start, unitVector, -horizontalComponent, true);
        ...
        it('get point by vertical component', () => {
            ...
            const unitVector = getUnitVectorByPointAndPoint(start, end);
            const result1 = getPointByVectorDirectionComponent(start, unitVector, verticalComponent, false);
            expect(result1[0]).toEqual(100);
            expect(result1[1]).toEqual(50);
            const result2 = getPointByVectorDirectionComponent(start, unitVector, -verticalComponent, false);
